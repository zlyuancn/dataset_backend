...
### 方案 5️⃣（✅ 最终选择）：**8MB Redis String 分片**

#### 🔧 核心设计：
| 组件         | 说明                                                              |
| ------------ | ----------------------------------------------------------------- |
| **数据分片** | 将数据每约 8MB 分为一个 chunk, 一个索引数据不能同时存在多个分片中 |
| **元数据**   | 记录每个 chunk 的最后一条数据索引                                 |
| **查询流程** | 1. 加载元数据 → 2. 二分查找分片 → 3. 加载分片 → 4. 内存解析出数据 |
| **缓存优化** | Query Service 本地 LRU 缓存元数据和已加载的分片数据（8MB/块）     |
| **加载优化** | 根据请求的数据索引提前缓存下一个分片                              |
| **压缩存储** | 分片数据使用Lz4压缩后存 Redis，节省内存                           |
...
| **Key 数合理** | 10GB → ~1280 Keys（比方案 2️⃣ 减少 96%）       |
...
| C. 8MB String + 元数据   | 按字节分片 + 解析      | ≈120MB	5~20ms | 25~30    | 13     | ⚠️ 过度设计  |
...
- 没有`LREM`/`LSET`/`LRANGE`操作. 由于无法使用`LRANGE`也就无法提前缓存数据到本地. (当前业务满足)
...
我们通过**系统性评估多种存储模型**，最终选择 “**8MB Redis String 分片**” 方案，因为它：
...
在**小场景**的情况下**文件大小 <= 100M**. 应该采用**单 Redis List**方案, 但是注意要求**redis版本≥4.0**. 但是考虑到我们为了支持更大的数据量采用了**8MB Redis String 分片**方案. 没必要再去实现一套, 且**单 Redis List**方案的读取性能要低一些, 且对Redis有更大的负载.
